...
from app.models import EnvelopeBase, NewsItem, ResolveResult
...
class MangaNewsService:
    ...
    def _envelope(self, payload: dict, entry, *, cached: bool, partial: bool, warnings: list[str], found: bool = True) -> dict:
    ...
    async def search(self, query: str, kind: Literal['series', 'volume', 'all'], mode: Literal['best', 'all'], limit: int) -> dict:
    ...
    async def resolve_search(self, query: str, kind: Literal['series', 'volume', 'all'], limit: int = 5) -> dict:
        payload, entry, cached, partial, warnings = await self._search_results(query=query, kind=kind, mode='all', limit=max(limit, 1))
        candidates = list(payload.get('data', []))
        ...
        return self._envelope({'data': data, 'source_url': payload.get('source_url')}, entry, cached=cached, partial=partial, warnings=warnings, found=best is not None)

    async def get_series(self, *, slug: str | None = None, url: str | None = None) -> dict:
    ...
    async def get_volume(self, *, series_slug: str | None = None, volume_slug: str | None = None, url: str | None = None) -> dict:
    ...
    async def get_global_news(self, *, limit: int) -> dict:
    ...
    async def get_series_news(self, *, slug: str, limit: int) -> dict:
    ...
    async def get_volume_news(self, *, series_slug: str | None = None, volume_slug: str | None = None, url: str | None = None, limit: int) -> dict:
        target_url = self._resolve_volume_url(series_slug=series_slug, volume_slug=volume_slug, url=url)
    ...
    async def _get_news_page(self, *, target_url: str, cache_namespace: str, ttl: int, limit: int) -> dict:
